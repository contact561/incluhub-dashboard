...
export function AssistantAvailabilityForm({
  portfolioOutputId,
  leaderPortfolioType,
}: {
  portfolioOutputId: string;
  leaderPortfolioType: StudentCategory;
}) {
  const [state, action, pending] = useActionState<SaveAvailabilityState, FormData>(saveStudioAvailabilityAction, {});
  const leaderLabel = STUDENT_CATEGORY_LABELS[leaderPortfolioType];

  return (
    <section className="mt-4 rounded-[var(--radius-control)] border border-border-default bg-surface-muted p-4">
      <h3 className="font-medium text-text-primary">When are you available?</h3>
      <p className="mt-1 text-sm text-text-muted">
        When are you available to support your teammate&apos;s {leaderLabel}{" "}
        shoot? Share the dates and timings that work for you. The leader can
        book only after both assistants respond.
      </p>
      <form action={action} className="mt-4 space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Input
            id={`availability-date-${portfolioOutputId}`}
            name="booking_date"
            type="date"
        ...
        <details className="rounded-lg border border-border-default bg-surface-card">
          ...
          <div className="grid gap-2 border-t border-border-default p-3 sm:grid-cols-2">
            {STUDIO_SLOT_CODES.map((slot) => (
              <label
                key={slot}
                className="flex min-h-11 items-center gap-2 rounded-lg border border-border-default px-3 text-sm"
              >
        ...
        <Button type="submit" disabled={pending}>
          {pending ? "Saving…" : "Share availability"}
        </Button>
      </form>
    </section>
  );
